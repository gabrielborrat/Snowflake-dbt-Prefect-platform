...
|-------|---------|-----------|
| `generate_surrogate_key.sql` | Wrapper around `dbt_utils.generate_surrogate_key()` for consistency | Zagni Ch.8, Ch.13 |
| `cents_to_dollars.sql` (if needed) | Convert monetary amounts (DRY pattern from Machado Ch.2) | Machado Ch.2 |
...
|------|-------------------|
| `assert_no_future_transactions.sql` | No `transaction_date` > `CURRENT_DATE()` |
| `assert_positive_prices.sql` | `close_price > 0` in `fact_daily_prices` |
| `assert_exchange_rates_positive.sql` | `exchange_rate > 0` in `fact_exchange_rates` |
| `assert_fact_transaction_amount_matches_staging.sql` | Row count sanity check between staging and mart |
...
dbt source freshness               # Source freshness checks
```

**Deliverables:**
- [ ] Generic tests declared for all models in YAML (not_null, unique, relationships, accepted_values)
- [ ] 4+ singular tests written and passing
- [ ] `dbt test` returns 0 failures
- [ ] Source freshness passing
